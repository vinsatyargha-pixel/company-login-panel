...
export default function ReviewBreakdownTransactionPage() {
  ...
  const generateHourlyData = (date, asset, status) => {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      // Chat Volume (setiap chat dihargai Rp 50.000)
  ...
  const fetchData = async () => {
    try {
      ...
      // Hitung summary
      const chatVolume = data.reduce((sum, item) => sum + item.chatVolume, 0);
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-[#1A2F4A] p-4 rounded-lg border border-[#FFD700]/30">
          ...
          <div className="flex justify-between text-xs mt-2">
            <span className="text-yellow-400">CS: {formatIDR(summaryData.chatVolume)}</span>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {traficData.map((item, idx) => {
                ...
                const chatPercentage = (item.chatVolume / item.totalVolume) * 100;
                ...
                return (
                  <tr key={idx} className="border-b border-[#FFD700]/10 hover:bg-[#0B1A33]/50 transition-colors">
                    ...
                    {/* CHAT DATA - Volume dalam IDR */}
                    <td className="px-4 py-3 text-right text-yellow-400 border-r border-[#FFD700]/10">{formatIDR(item.chatVolume)}</td>
