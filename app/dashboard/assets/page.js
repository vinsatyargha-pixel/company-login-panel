    ...
    try {
      ...
      for (const asset of (assetsData || [])) {
        ...
        summaries.push({
          ...
          trx: (deposits?.length || 0) + (withdrawals?.length || 0),
          amt: totalDeposit + totalWithdrawal,
  ...
  return (
    <div className="p-6 w-full min-h-screen bg-[#0B1A33] text-white">
      <div className="mb-8">
        ...
        <p className="text-[#A7D8FF] mt-2">Monitoring performa asset</p>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-[#FFD700]/30 bg-[#0B1A33]">
              <th className="px-4 py-3 text-left text-[#FFD700] font-bold">ASSET</th>
              <th className="px-4 py-3 text-right text-[#FFD700] font-bold">TRX</th>
              ...
              <th className="px-4 py-3 text-right text-[#FFD700] font-bold">%</th>
          ...
          <tbody>
            {assetSummaries.map((asset, idx) => (
              <tr key={asset.asset_id} className={`border-b border-[#FFD700]/20 hover:bg-[#1A2F4A]/50 transition-colors ${idx % 2 === 0 ? 'bg-[#1A2F4A]/30' : ''}`}>
                <td className="px-4 py-3 font-bold text-[#FFD700]">{asset.asset_code}</td>
                <td className="px-4 py-3 text-right text-white">{formatNumber(asset.trx)}</td>
                <td className="px-4 py-3 text-right text-white">{formatIDR(asset.amt)}</td>
                <td className="px-4 py-3 text-right text-white">{formatNumber(asset.new_register)}</td>
                <td className="px-4 py-3 text-right text-white">{formatNumber(asset.new_member_deposit)}</td>
                <td className="px-4 py-3 text-right text-white">{asset.percentage.toFixed(2)}%</td>
                <td className="px-4 py-3 text-right text-green-400">{formatIDR(asset.total_deposit)}</td>
                <td className="px-4 py-3 text-right text-red-400">{formatIDR(asset.total_withdrawal)}</td>
                <td className="px-4 py-3 text-right text-white">{formatNumber(asset.active_member)}</td>
                <td className="px-4 py-3 text-right text-blue-400">{formatIDR(asset.turnover)}</td>
                <td className={`px-4 py-3 text-right ${asset.winlose <= 0 ? 'text-green-400' : 'text-red-400'}`}>{formatIDR(asset.winlose)}</td>
                <td className="px-4 py-3 text-right text-white">{formatIDR(asset.bonus)}</td>
                <td className="px-4 py-3 text-right text-white">{formatIDR(asset.commission)}</td>
                <td className="px-4 py-3 text-right text-white">{formatIDR(asset.cashback)}</td>
                <td className="px-4 py-3 text-right text-white">{formatIDR(asset.adjustment)}</td>
                <td className="px-4 py-3 text-right text-white">{formatIDR(asset.referral)}</td>
              </tr>
            ))}
          </tbody>
        </table>
