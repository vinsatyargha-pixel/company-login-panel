...
export default function ChatCSPage() {
  ...
  const processFile = async (): Promise<void> => {
    ...
    try {
      ...
      const findIndex = (keyword: string): number => headers.findIndex((h: string) => h && h.toString().toLowerCase().includes(keyword.toLowerCase()))
      ...
      if (validData.length === 0) throw new Error('Tidak ada data valid dalam file')
  ...
  return (
    <div className="p-6 min-h-screen bg-[#0B1A33] text-white">
      ...
      <div className="bg-[#1A2F4A] p-4 rounded-lg border border-[#FFD700]/30 mb-6 flex flex-wrap gap-4 items-center">
        ...
        <select className="bg-[#0B1A33] border border-[#FFD700]/30 rounded-lg px-4 py-2 text-white" value={selectedAsset} onChange={(e) => setSelectedAsset(e.target.value)}>
          <option value="all">SEMUA WEBSITE</option>
          {assets.map(asset => <option key={asset.id} value={asset.id}>{asset.asset_name}</option>)}
        </select>
        <div className="ml-auto text-[#A7D8FF]">Total: <span className="text-[#FFD700] font-bold">{uploads.length}</span> file</div>
      </div>
      ...
      <div className="bg-[#1A2F4A] rounded-lg border border-[#FFD700]/30 overflow-hidden">
        <table className="w-full">
          <thead className="bg-[#0B1A33] border-b border-[#FFD700]/30">
            <tr><th className="px-4 py-3 text-left text-[#FFD700]">Tanggal</th><th className="px-4 py-3 text-left text-[#FFD700]">Website</th><th className="px-4 py-3 text-left text-[#FFD700]">File</th><th className="px-4 py-3 text-left text-[#FFD700]">Jumlah Chat</th><th className="px-4 py-3 text-left text-[#FFD700]">Status</th></tr>
          </thead>
          <tbody>
            {uploads.length > 0 ? uploads.map(item => (
              <tr key={item.id} className="border-b border-[#FFD700]/10 hover:bg-[#0B1A33]/50">
                ...
                <td className="px-4 py-3"><span className={`px-2 py-1 rounded text-xs ${getStatusColor(item.status)}`}>{item.status}</span></td>
              </tr>
